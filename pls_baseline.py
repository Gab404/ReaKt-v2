...
pls_baseline.py
===============
Linear, static PLS regression baseline for the ReaKt-v2 bioprocess soft-sensor
benchmark (IndPenSim V3 — Penicillin prediction from Raman spectra).

Model
-----
  sklearn.cross_decomposition.PLSRegression
    n_components = 4   (previously optimised latent-space size)
    scale        = True

Input
-----
  2001-point Raman spectra, preprocessed with a Savitzky-Golay first-derivative
  filter (SG d=1, window=15, poly=2).  Each time step is treated as an
  independent sample  →  X shape: [n_samples, N_RAMAN_COLS].
...
----------
  ...
  evaluation methodology used for PI-LSTM and Neural-ODE in this benchmark.
...
Usage
-----
  python pls_baseline.py                          # uses defaults below
  python pls_baseline.py --csv path/to/data.csv
"""
...
# PLS hyperparameters
N_COMPONENTS: int = 4       # latent components (previously optimised)

# Raman spectral window
# The benchmark uses the first 2001 wavenumber channels (400–2400 cm⁻¹ @1 cm⁻¹
# resolution).  RAMAN_N_COLS in dataset.py is 2200; we deliberately cap at 2001.
N_RAMAN_COLS: int = 2001    # spectral dimensionality fed to PLS
...
# Train / val split (mirrors the PI-LSTM random split for a fair comparison)
...
def load_batches(csv_path: str) -> List[Dict]:
    ...
    print(f"[DATA]  Raw shape: {df.shape[0]:,} rows × {df.shape[1]} cols")
...
def split_batches(
    ...
    """
    Replicate the PI-LSTM random split (seed=42) for a strict apples-to-apples
    comparison.
...
def run_cross_validation(
    ...
    n_components             : PLS latent components
    ...
    y_cv_pred = cross_val_predict(pls, X_train, y_train, cv=cv, **cv_kwargs)
...
def fit_pls(
    ...
    """
    Fit PLS on the full training/calibration set.
    ...
    Returns
    -------
    Fitted PLSRegression model
    """
    print(f"\n[FIT]  Fitting PLSRegression(n_components={n_components}, scale=True) "
...
def save_model(
    ...
    """
    Persist the fitted PLS model (and optional metadata) to disk using joblib.

    The saved artefact is a dict with keys:
      "model"    : the fitted PLSRegression object
      "metadata" : dict with hyperparameters, RMSECV, and metric results

    Parameters
    ----------
    model    : fitted PLSRegression
...
def _predict_batch_at_sparse_points(
    ...
    """
    Run the PLS model on a single batch and collect predictions only at the
    sparse offline measurement time-points  (i.e., where  y_pen_sparse ≠ NaN).

    This exactly mirrors the evaluation methodology for PI-LSTM / Neural-ODE.

    Returns
    -------
    y_true : (n_sparse,) — measured Penicillin values
    y_pred : (n_sparse,) — PLS predictions at those time-points
...
def evaluate_split(
    ...
    """
    Evaluate PLS on a list of batches (FF-val or fault batches).
...
def print_comparison_table(
    ...
    sep = "-" * 64
    print(f"\n{sep}")
    print(f"  PLS BASELINE -- BENCHMARK SUMMARY  (n_components={N_COMPONENTS})")
    print(sep)
    print(f"  Calibration")
    ...
    print(f"  {'Model':<20} | {'Pen RMSE (val)':>14} | {'Pen R2 (val)':>12} | "
...
def main(csv_path: str = CSV_PATH) -> None:
    """
    End-to-end PLS baseline pipeline:
      ...
      4.  Fit final PLS on all calibration batches
      5.  Evaluate on FF-val and fault batches
      6.  Print benchmark comparison table
      7.  Save fitted model to CHECKPOINT_PATH
    """
    # ── 1. Load data ─────────────────────────────────────────────────────────
    ...
    # consistent with how PI-LSTM and Neural-ODE are trained.
    ...
    # ── 5. Fit final model ────────────────────────────────────────────────────
    pls = fit_pls(X_train, y_train)

    # ── 6. Evaluate — FF-val ─────────────────────────────────────────────────
    ffval_metrics = evaluate_split(pls, val_batches, split_name="FF-Validation")

    # ── 7. Evaluate — fault batches ───────────────────────────────────────────
    fault_mean_rmse, per_batch = evaluate_fault_batches(pls, fault_batches)

    # ── 8. Benchmark summary ──────────────────────────────────────────────────
    print_comparison_table(rmsecv, ffval_metrics, fault_mean_rmse, per_batch)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="PLS baseline for ReaKt-v2 Penicillin soft-sensor benchmark.",
