"""
src/algorithms/cdae_pi_lstm.py
==============================
CDAEPILSTMAlgorithm — full fit/evaluate pipeline for the CDAE-PI-LSTM.
...
---------
  CSV  →  PenicillinDataModule (use_raman=True, encoder=CDAERamanEncoderV2)
       ...
       →  CDAEPILSTMAlgorithm._build_dataset()
...
Feature philosophy
------------------
  This model uses ONLY the 64D CDAE latents extracted from the Raman spectra.
  No process variables (aeration, temperature, …) are included.  This makes
  it a pure spectroscopic soft-sensor — a direct benchmark of "how much
  fermentation information is encoded in the Raman signal alone".

  The existing PILSTMAlgorithm (process_23) and PILSTMAlgorithm + Raman
  (process_23 + raman_64 = 87-D) are the comparators.
...
class CDAEPILSTMAlgorithm(BaseAlgorithm):
    """
    Physics-Informed LSTM algorithm driven exclusively by CDAE Raman latents.

    Config sections expected
    ------------------------
    data.use_raman          : true       (required — triggers CDAE encoding)
    data.raman_encoder_type : "cdae_v2"  (selects CDAERamanEncoderV2)
    data.raman_ckpt         : path to checkpoints/cdae_best.pt
    data.raman_scaler       : path to checkpoints/cdae_scaler.joblib
    data.raman_latent_dim   : 64         (CDAE output dimensionality)
    ...
    def _feature_cols(self) -> List[str]:
        """
        Return the 64 CDAE latent column names.

        These columns are populated by CDAERamanEncoderV2.encode_dataframe()
    ...
    def _fit_scalers(self, train_batches) -> ScalerBundle:
        """
        Fit MinMaxScalers for CDAE latent features and targets.
    ...
    def _train_epoch(
        ...
        For each batch:
          ...
          2. compute_cdae_pinn_loss → data MSE + λ · mass-balance residual
        ...
        for x_b, y_b in loader:
            # x_b : (B, seq_len, 64)   — MinMaxScaled CDAE latents
    ...
    def evaluate(self, batches) -> Dict[str, Any]:
        ...
        5. Compute RMSE / MAE / R² ONLY at the sparse offline measurement
        ...
        print("  [Biomass   ] N/A (CDAE-PI-LSTM predicts Penicillin only)")
...
# ── Register ──────────────────────────────────────────────────────────────────
REGISTRY[CDAEPILSTMAlgorithm.name] = CDAEPILSTMAlgorithm
