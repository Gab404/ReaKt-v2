"""
src/algorithms/pls_pi_lstm.py
=============================
PLSPILSTMAlgorithm -- full fit/evaluate pipeline for the PLS-PI-LSTM.

Data flow
---------
  CSV  ->  PenicillinDataModule (use_raman=True, encoder=PLSRamanEncoderV1)
       ->  batch DataFrames  with  _raman_0 ... _raman_(K-1)  columns
           (K-dimensional PLS scores; default K=4)
       ->  PLSPILSTMAlgorithm._build_dataset()
              MinMaxScale the K-D PLS scores -> X windows  (seq_len, K)
...
Why this exists
---------------
  This is the direct PLS counterpart of CDAEPILSTMAlgorithm and the
  supervised-linear sibling of PCAPILSTMAlgorithm.  It plugs a frozen
  PLS Raman encoder (linear, *supervised* via Penicillin during fit)
  into the same LSTM + physics-loss head used by CDAE-PI-LSTM and
  PCA-PI-LSTM.  The intended scientific comparison is:

      static    : PLS  vs  PCA-PCR     -> supervised vs unsupervised
                                          linear reduction at K=4
      temporal  : PLS-PI-LSTM  vs  PCA-PI-LSTM
                                       -> supervised vs unsupervised
                                          linear encoder feeding the
                                          same LSTM+physics head
      temporal  : CDAE-PI-LSTM  vs  PLS-PI-LSTM
                                       -> non-linear (CDAE 64-D) vs
                                          supervised linear (PLS 4-D)
                                          encoder

Feature philosophy
------------------
  This model uses ONLY the K-D PLS scores extracted from the Raman spectra.
...
-----------
  ...
  PLS-vs-PCA-vs-CDAE comparison strictly downstream-equivalent.
"""
...
class PLSPILSTMAlgorithm(BaseAlgorithm):
    """
    Physics-Informed LSTM algorithm driven exclusively by PLS Raman scores.

    Config sections expected
    ------------------------
    data.use_raman          : true        (required -- triggers PLS encoding)
    data.raman_encoder_type : "pls_v2"    (selects PLSRamanEncoderV1)
    data.raman_ckpt         : path to checkpoints/pls_best.joblib
    data.raman_scaler       : path to checkpoints/pls_scaler.joblib
    data.raman_latent_dim   : 4           (number of PLS components)
    ...
    def _feature_cols(self) -> List[str]:
        """
        Return the K PLS latent column names (K = raman_latent_dim, default 4).

        These columns are populated by PLSRamanEncoderV1.encode_dataframe()
    ...
    def _build_model(self) -> CDAEPILSTMModel:
        ...
        return CDAEPILSTMModel(
            input_size=n_feat,        # K=4 for PLS-PI-LSTM (vs 64 for CDAE)
    ...
    def _fit_scalers(self, train_batches) -> ScalerBundle:
        """
        Fit MinMaxScalers for PLS latent features and targets.
    ...
    def _build_dataset(
        ...
        Each window:
          X : (seq_len, n_feat=K)  -- MinMaxScaled PLS scores
    ...
    def _train_epoch(
        ...
        for x_b, y_b in loader:
            # x_b : (B, seq_len, K)    -- MinMaxScaled PLS scores
    ...
    def evaluate(self, batches) -> Dict[str, Any]:
        ...
        print("  [Biomass   ] N/A (PLS-PI-LSTM predicts Penicillin only)")
...
# -- Register ---------------------------------------------------------------
REGISTRY[PLSPILSTMAlgorithm.name] = PLSPILSTMAlgorithm
